...
class OMNITRAX_OT_DetectionOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if (
            not os.path.isfile(yolo_cfg_path_temp)
            or yolo_cfg_path_temp.split(".")[-1] != "cfg"
        ):
            self.report(
                {"ERROR"}, "Provide the .cfg file of a suitable YOLOv3 or V4 model"
            )
            return {"CANCELLED"}

        yolo_weights_path_temp = bpy.path.abspath(context.scene.detection_weights_path)
        if (
            not os.path.isfile(yolo_weights_path_temp)
            or yolo_weights_path_temp.split(".")[-1] != "weights"
        ):
            self.report(
                {"ERROR"}, "Provide the .weights file of a trained YOLOv3 or V4 model"
            )
            return {"CANCELLED"}
        ...
        if context.scene.compute_device.split("_")[0] == "GPU":
            ...
            CUDA_match = check_CUDA_installation(
            ...
            if not CUDA_match:
                self.report(
                    {"ERROR"},
                    "No matching CUDA version found! Refer to console for full error message",
                )
                return {"CANCELLED"}
            ...
            if platform.system() != "Linux":
                ...
                if bpy.app.version_string == "2.92.0":
                    ...
                    self.report(
                        {"ERROR"},
                        "To use CPU inference, you need to use OmniTrax with Blender version 2.9.2!",
                    )
                    return {"CANCELLED"}
        ...
        try:
            ...
            for m, mask in enumerate(
                bpy.data.masks["Mask"].layers["MaskLayer"].splines
            ):
                print(mask)
                mask_individual = []
                for point in (
                    bpy.data.masks["Mask"].layers["MaskLayer"].splines[m].points
                ):
                    ...
                    [
                        [
                            max(
                                min(
                                    int(
                                        max((video_height - video_width), 0) / 2
                                        + x * video_width
                                    ),
                                    video_width,
                                ),
                            ...
                            max(
                                min(
                                    int(
                                        max((video_width - video_height), 0) / 2
                                        + video_height
                                        - y
                                        * video_height
        ...
        tracker_KF = Tracker(
            dist_thresh=context.scene.tracking_dist_thresh,
        ...
        if self.restart_tracking:
            ...
            for mname in clip.tracking.objects[0].tracks:
                ...
                try:
                    ...
                    tracker_KF.initialise_from_prior_state(
                        prior_state=[
                            track_id_temp,
                            marker_x,
        ...
        yolo_paths = YOLOader(
            cfg=yolo_cfg, weights=yolo_weights, data=yolo_data, names=yolo_names
        )

        yolo_paths.update_cfg(
            nw_width=bpy.context.scene.detection_network_width,
        ...
        if len(class_names) == 1:
            ...
            for c, cn in enumerate(class_names):
                class_colours[cn] = [
                    int((255 / len(class_names)) * (c + 1)),
                    int(255 - (255 / len(class_names)) * c),
                    0,
                ]
                class_id[cn] = c
        ...
        if not os.path.exists(yolo_paths.cfg):
            raise ValueError(
                "Invalid config path `" + os.path.abspath(yolo_paths.cfg) + "`"
            )
        if not os.path.exists(yolo_paths.weights):
            raise ValueError(
                "Invalid weight path `" + os.path.abspath(yolo_paths.weights) + "`"
            )
        if not os.path.exists(yolo_paths.data):
            raise ValueError(
                "Invalid data file path `" + os.path.abspath(yolo_paths.data) + "`"
            )
        if network is None:
            try:
                network, class_names, _ = darknet.load_network(
                    yolo_paths.cfg, yolo_paths.data, yolo_paths.weights, batch_size=1
                )
        ...
        while True:
            try:
                if tracker_continue:
                    if (
                        frame_counter
                        == context.scene.frame_end + 3 - executed_from_frame
                    ):
                ...
                if bpy.context.scene.detection_enforce_constant_size:
                    image = cvDrawBoxes(
                        detections,
                        frame_resized,
                        min_size=bpy.context.scene.detection_min_size,
                    ...
                    for i in range(len(tracker_KF.tracks)):
                        if len(tracker_KF.tracks[i].trace) > 1:
                            ...
                            if mname in track_classes:
                                track_classes[mname][
                                    bpy.context.scene.frame_current
                                ] = tracker_KF.tracks[i].predicted_class[-1]
                            else:
                                track_classes[mname] = {
                                    bpy.context.scene.frame_current: tracker_KF.tracks[
                                        i
                                    ].predicted_class[-1]
                                }

                            if mname not in track_colors:
                                track_colors[mname] = np.random.randint(
                                    low=100, high=255, size=3
                                ).tolist()

                            # draw direction of movement onto footage
                            x_t, y_t = tracker_KF.tracks[i].trace[-1]
                            tracker_KF_velocity = 5 * (
                                tracker_KF.tracks[i].trace[-1]
                                - tracker_KF.tracks[i].trace[-2]
                            )
                            x_t_future, y_t_future = (
                                tracker_KF.tracks[i].trace[-1]
                                + tracker_KF_velocity * 0.1
                            )
                            cv2.arrowedLine(
                                image,
                                (int(x_t), int(y_t)),
                                (int(x_t_future), int(y_t_future)),
                                (
                                    np.array(track_colors[mname])
                                    - np.array([70, 70, 70])
                                ).tolist(),
                                3,
                                tipLength=0.75,
                            ...
                            cv2.putText(
                                ...
                                (
                                    int(x2)
                                    - int(
                                        bpy.context.scene.detection_constant_size / 2
                                    ),
                                    int(y2) - bpy.context.scene.detection_constant_size,
                                ),
...
class EXPORT_OT_Operator(bpy.types.Operator):
    ...
    selected_only: BoolProperty(
        name="Selected Only", description="Export selected markers only", default=False
    )
    ...
    def execute(self, context):
        ...
        for clip in movieclips:
            ...
            if log:
                log_file.write(
                    "Starting movieclip {0} ({1} x {2} pixels)\n".format(
            ...
            for track in tracks:
                ...
                if not subdirs:
                    export_file = open(
                        path
                        + "{0}_{1}.csv".format(clip.name.split(".")[0], track.name),
                        "w",
                    )
                else:
                ...
                while i <= f_end:
                    ...
                    if marker:
                        ...
                        export_file.write(
                            "{0};{1};{2};{3}\n".format(
                                i, marker_x, marker_y, class_pred
                            )
                        )
                    else:
                        if log:
                            log_file.write(
                                "    Missing marker at frame {0}.\n".format(i)
                            )
                        success = False
                    i += 1
            ...
            if log:
                log_file.write(
                    "Finished movieclip {0} in {1:.4f} s\n\n".format(
        ...
        if log:
            ...
            log_file.write(
                "Export finished ({0:.4f} s)".format(time.time() - time_start)
            )
            log_file.close()

        self.report(
            {"INFO"}, "Export done ({0:.4f} s)".format(time.time() - time_start)
        )
...
class EXPORT_OT_AdvancedSampleExportOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        try:
            ...
            print(
                "You need to load and track a video, before extracting any samples!\n"
            )
            return {"FINISHED"}
            ...
            for frame_id in range(first_frame, last_frames):
                ...
                try:
                    if marker:
                        ...
                        print(
                            "Frame:", frame_id, " : ", "X", marker_x, ",", "Y", marker_y
                        )
                        ...
                        if ret:
                            ...
                            track_input_img = np.zeros(
                                [
                                    context.scene.exp_ase_input_y,
                                    context.scene.exp_ase_input_x,
                                    3,
                                ],
                                dtype=np.uint8,
                            )
                            ...
                            if context.scene.exp_ase_fixed_input_bounding_box_size:
                                true_min_x = marker_x - int(
                                    context.scene.exp_ase_input_x / 2
                                )
                                true_max_x = marker_x + int(
                                    context.scene.exp_ase_input_x / 2
                                )
                                true_min_y = (
                                    clip_height
                                    - marker_y
                                    - int(context.scene.exp_ase_input_y / 2)
                                )
                                true_max_y = (
                                    clip_height
                                    - marker_y
                                ...
                                track_input_img[
                                    y_min_offset:y_max_offset, x_min_offset:x_max_offset
                                ] = frame_cropped
                                ...
                                if context.scene.exp_ase_padding:
                                    if true_width >= true_height:
                                        border_height = max(
                                            [int((true_width - true_height) / 2), 0]
                                        )
                                        track_input_img = np.zeros(
                                            [true_width, true_width, 3], dtype=np.uint8
                                        )

                                        track_input_img[
                                            border_height : true_height + border_height,
                                            :,
                                        ] = frame_temp[
                                            true_min_y:true_max_y, true_min_x:true_max_x
                                        ]
                                    else:
                                        border_width = max(
                                            [
                                                int(
                                                    abs((true_height - true_width)) / 2
                                                ),
                                                0,
                                            ]
                                        )
                                        ...
                                        track_input_img[
                                            :, border_width : true_width + border_width
                                        ] = frame_temp[
                                            true_min_y:true_max_y, true_min_x:true_max_x
                                        ]

                                else:
                                    track_input_img = frame_temp[
                                        true_min_y:true_max_y, true_min_x:true_max_x
                                    ]

                        if context.scene.exp_ase_fixed_output_bounding_box_size:
                            track_input_img = cv2.resize(
                                track_input_img,
                                [
                        ...
                        if context.scene.exp_ase_grayscale:
                            track_input_img = cv2.cvtColor(
                                track_input_img, cv2.COLOR_BGR2GRAY
                            )
                        ...
                        out_path = (
                            str(
                                os.path.join(
                                    os.path.abspath(context.scene.exp_ase_path),
                                    os.path.basename(
                                        bpy.context.edit_movieclip.filepath[:-4]
                                    ),
                                )
                            )
                            + "_"
