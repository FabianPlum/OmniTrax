...
class YoloTracker:
	def __init__(
		...
		net_data,
		video_path,
		detection_activation_threshold=0.5,
		detection_nms=0.45,
		tracker_dist_thresh=100,
		tracker_max_frames_to_skip=20,
		tracker_max_trace_length=200,
		tracker_track_id_count=0,
		prior_tracks=[],
		prior_classes=[],
		tracker_use_kf=True,
		tracker_std_acc=5,
		tracker_x_std_meas=0.25,
		tracker_y_std_meas=0.25,
		dt=0,
		frame_start=0,
		frame_end=-1,
		continue_tracking=False,
		detection_min_size=50,
		detection_constant_size=100,
		detection_enforce_constant_size=False,
	):
		...
		self.tracker_KF = Tracker(
			dist_thresh=tracker_dist_thresh,
		...
		if not os.path.exists(self.net_cfg):
			raise ValueError(
				"Invalid config path `" + os.path.abspath(self.net_cfg) + "`"
			)
		if not os.path.exists(self.net_weight):
			raise ValueError(
				"Invalid weight path `" + os.path.abspath(self.net_weight) + "`"
			)
		if not os.path.exists(self.net_data):
			raise ValueError(
				"Invalid data file path `" + os.path.abspath(self.net_data) + "`"
			)
	...
	def run_inference(
		...
		self.network, self.class_names, self.class_colours = darknet.load_network(
			self.net_cfg, self.net_data, self.net_weight, batch_size=1
		)
		...
		while True:
			...
			frame_resized = cv2.resize(
				frame_rgb, (video_width, video_height), interpolation=cv2.INTER_LINEAR
			)
			...
			for label, confidence, bbox in detections:
				if (
					bbox[2] >= self.detection_min_size
					and bbox[3] >= self.detection_min_size
				):
				...
				for i in range(len(self.tracker_KF.tracks)):
					if len(self.tracker_KF.tracks[i].trace) > 1:
						...
						if mname in track_classes:
							...
							track_classes[mname] = {
								self.frame_start
								+ frame_counter: self.tracker_KF.tracks[
									i
								].predicted_class[-1]
							}

						if mname not in self.track_colors:
							self.track_colors[mname] = np.random.randint(
						...
						tracker_KF_velocity = 5 * (
							self.tracker_KF.tracks[i].trace[-1]
							- self.tracker_KF.tracks[i].trace[-2]
						)
						...
						cv2.arrowedLine(
							...
							(
								np.array(self.track_colors[mname])
								- np.array([70, 70, 70])
							).tolist(),
							3,
							tipLength=0.75,
						...
						for j in range(len(self.tracker_KF.tracks[i].trace) - 1):
							hind_sight_frame = (
								self.frame_start
								+ frame_counter
								- len(self.tracker_KF.tracks[i].trace)
								+ j
							)
						...
						cv2.putText(
							...
							(
								...
								int(y2) - self.detection_constant_size,
							),
...
if __name__ == "__main__":
	...
	ap.add_argument(
		"-vs", "--frame_start", type=int, default=0, help="First frame to analyze"
	)
	ap.add_argument(
		"-ve", "--frame_end", type=int, default=-1, help="Last frame to analyze"
	)
	...
	ap.add_argument(
		"-detthresh",
		"--detection_activation_threshold",
		type=float,
		default=0.5,
	...
	ap.add_argument(
		"-detconst",
		"--detection_enforce_constant_size",
		type=bool,
		default=False,
	...
	YT = YoloTracker(
		...
		detection_nms=args["detection_nms"],
		tracker_dist_thresh=args["tracker_dist_thresh"],
		...
		prior_tracks=args["prior_tracks"],
		prior_classes=args["prior_classes"],
		tracker_use_kf=args["tracker_use_kf"],
		tracker_std_acc=args["tracker_std_acc"],
	...
	output = YT.run_inference(
		export_video=args["write_video"],
