...
class OMNITRAX_OT_DetectionOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        video_out = cv2.VideoWriter(video_output,cv2.VideoWriter_fourcc('M','J','P','G'), fps, (int(cap.get(3)),int(cap.get(4))))
        ...
        if len(class_names) == 1:
            ...
            for c, cn in enumerate(class_names):
                class_colours[cn] = [int((255/len(class_names))*(c+1)),int(255 - (255/len(class_names))*c),0]
                class_id[cn] = c
                ...
                for i in range(len(tracker.tracks)):
                    if (len(tracker.tracks[i].trace) > 1):
                        ...
                        if mname in track_classes:
                            track_classes[mname][bpy.context.scene.frame_current] = tracker.tracks[i].predicted_class[-1]
                        else:
                            track_classes[mname]= {bpy.context.scene.frame_current: tracker.tracks[i].predicted_class[-1]}
                        ...
                        cv2.arrowedLine(image, (int(x_t), int(y_t)), (int(x_t_future), int(y_t_future)),
                                 (np.array(track_colors[mname])-np.array([70,70,70])).tolist(), 3, tipLength = 0.75)
